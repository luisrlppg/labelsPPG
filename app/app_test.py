# app_test.py - Version with real Odoo data
import os
import io
import base64
import logging
...
# ---------------------------
# Real Odoo data functions
# ---------------------------
@lru_cache(maxsize=1)
def fetch_clientes():
    """Obtiene clientes reales desde Odoo."""
    try:
        ...
        clientes = models.execute_kw(
            ...
            {'fields': ['name', 'phone', 'street', 'street2', 'city', 'zip', 'email'], 'limit': 100}
        )
        logger.info(f"Obtenidos {len(clientes)} clientes desde Odoo")
        return clientes
    except Exception as e:
        logger.error(f"Error conectando a Odoo para clientes: {e}")
        return []

@lru_cache(maxsize=1)
def fetch_productos():
    """Obtiene productos reales desde Odoo."""
    try:
        models, uid = connect_odoo()
        productos = models.execute_kw(
            ...
            {'fields': ['display_name', 'image_1920'], 'limit': 100}
        )
        logger.info(f"Obtenidos {len(productos)} productos desde Odoo")
        return productos
    except Exception as e:
        logger.error(f"Error conectando a Odoo para productos: {e}")
        return []
